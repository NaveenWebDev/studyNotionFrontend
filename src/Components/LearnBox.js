...
const LearnBox = () => {
  ...
  return (
    ...
    <div className='max-w-[1200px] w-[90%] m-auto'>
        ...
        <div className="tabs flex justify-between m-auto w-[60%] m-auto bg-richblack-800 rounded-full p-1 mt-3">
            {
                tabNames.map((val, i)=>{
                    return(
                    <button key={i} onClick={()=>setValue(val)} className={` ${currentTag == val? 'bg-richblack-900' :null}  py-1 px-5 rounded-full`}>{val}</button>
                    )
                })
            }
        ...
        <div className="mt-[3rem] flex justify-between flex-wrap ">
        {
            ...
            <div className='relative p-5 w-[30%] bg-white '>
                <p className='text-black font-semibold text-xl'>{val.heading}</p>
